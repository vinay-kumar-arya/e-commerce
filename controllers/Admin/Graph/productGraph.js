...
export const productGraph = async (req, res) => {
  try {
    ...
    const prepareStats = (groupBy, startMoment, totalUnits, unitType) => {
      ...
      for (let i = 0; i < totalUnits; i++) {
        ...
        const totalProducts = stats.totalActive + stats.totalInactive + stats.totalDeleted;
        cumulativeTotal += totalProducts;
    ...
    console.error("Product Graph Error:", error.message);
    return res.status(500).json({
      success: false,
      message: "Cannot Fetch Product Graph",
    });
  }
};
