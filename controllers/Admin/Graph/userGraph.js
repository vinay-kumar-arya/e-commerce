...
export const userGraph = async (req, res) => {
  try {
    ...
    const prepareStats = (groupBy, startMoment, totalUnits, unitType) => {
      ...
      for (let i = 0; i < totalUnits; i++) {
        ...
        const totalUsers = stats.totalActive + stats.totalInactive + stats.totalDeleted;
        cumulativeTotal += totalUsers;
    ...
    console.error("User Graph Error:", error.message);
    return res.status(500).json({
      success: false,
      message: "Cannot Fetch User Graph",
    });
  }
};
